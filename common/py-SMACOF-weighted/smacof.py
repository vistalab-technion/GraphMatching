...
def run_smacof(weights, lengths, target_dimension = 3, max_iterations = 1000):
    ...
    X = smacof_numerical.smacof_embed(target_dimension = target_dimension, weights = weights, lengths = lengths,
                                      Vplus = Vplus, maximum_iterations = max_iterations)
    print("...done.")
...
def test_smacof(filename, target_dimension = 3):
    ...
    X = smacof_numerical.smacof_embed(target_dimension = target_dimension, weights = weights, lengths = lengths,
                                      vplus = Vplus, maximum_iterations = maximum_iterations)
    

    return lengths, weights, Vplus, X

if __name__ == "__main__":
    lengths = np.zeros((9, 1))
    # lengths[]
    l = lengths.shape[0]
    numvertices = int(math.sqrt(l))
    lengths = lengths.reshape((numvertices, numvertices))
    lengths[0][1] = lengths[1][0] = 1
    lengths[1][2] = lengths[2][1] = 1
    lengths[0][2] = lengths[2][0] = 1

    print(lengths)

    weights = np.ones(lengths.shape) # np.vectorize(lambda x: math.exp(-x))(lengths)
    ...
    X = smacof_numerical.smacof_embed(numvertices=numvertices, target_dimension=target_dimension, weights=weights, lengths=lengths,
                                      Vplus=Vplus, maximum_iterations=maximum_iterations)
    print(X)

    plt.scatter(X[:, 0], X[:, 1], c=['red', 'blue', 'yellow'])
    plt.grid()
