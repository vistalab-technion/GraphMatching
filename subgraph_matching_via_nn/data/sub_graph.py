...
class SubGraph:
    ...
    @property
    def A_full(self):
        return get_graph_adj_mat_as_tensor(self.G)

    @property
    def A_sub(self):
        return get_graph_adj_mat_as_tensor(self.G_sub)
    ...
    def w_gt(self):
        ...
        return (tensor(self.node_indicator)[:, None].float() / tensor(self.node_indicator).sum()).type(
        TORCH_DTYPE)
