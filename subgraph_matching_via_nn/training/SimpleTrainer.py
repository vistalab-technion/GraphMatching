...
from subgraph_matching_via_nn.training.localization_grad_distance import \
    LocalizationGradDistance
from subgraph_matching_via_nn.training.losses import get_pairs_batch_aggregated_distance


class SimilarityMetricTrainer:

    def __init__(self, graph_similarity_module, dump_base_path: str,
    ...
    def train(self, train_samples_list, val_samples_list, new_samples_amount=0):
        ...
        dump_path = os.path.join(self.dump_base_path, str(time.time()))
    ...
    def _train_loop(self, model, train_loader, val_loader, dump_path=None):
        all_train_losses = []
        all_val_losses = []
    ...
    # TODO: Pair_Sample_Info is based on nx.graph. for better performance, we could swith to a pair class
    # (e.g. Pair_Sample_Edit_Distance_Info) containing torch_geometric_data graph,
    # which can be composed into a batch of graphs
    # batch_size should be tweaked accordingly
    def get_data_loaders(self, train_set: List[Pair_Sample_Info],
        ...
        """
        batch_size = 1  # batching is not supported for nx.graph-s
        ...
        train_loader.collate_fn = lambda \
                samples_batch: SimilarityMetricTrainer.custom_collate(
            [sample[0] for sample in samples_batch])

        val_loader = tg.data.DataLoader(val_set, batch_size=batch_size)
        val_loader.collate_fn = SimilarityMetricTrainer.custom_collate
    ...
    def custom_collate(samples_batch):
        ...
        # TODO: use graph types (instead of S2VGrapn and nx.graph) that support batching
    ...
    def calculate_loss_for_batch(self,
                                 batch: Tuple[Pair_Sample_Info, List[Pair_Sample_Info]],
                                 is_train: bool) \
            -> torch.Tensor:
        """
        ...
        # TODO: use graph types (instead of S2VGraph and nx.graph) that support batching
        if is_train:
            # emb_distances = self.graph_similarity_module.forward_batch(batch)
            emb_distances = self.graph_similarity_module.forward(sample.masked_graph,
                                                                 sample.subgraph)
            return self.get_aggregated_pairs_batch_distance(emb_distances, batch)
        else:
            with torch.no_grad():
                # emb_distances = self.graph_similarity_module.forward_batch(batch)
                emb_distances = self.graph_similarity_module.forward(
                    sample.masked_graph, sample.subgraph)
                return self.get_aggregated_pairs_batch_distance(emb_distances, batch)
