...
class GK_WL():
    ...
    def compare_list(self, graph_list, h=1, node_label=True):
        ...
        for i in range(n):
            lists[i] = nx.to_numpy_array(graph_list[i])
        ...
        n_max += 2 #for self-loop support [there is a BUG apparently, self loop even considered as additional 2 in the node degree! TODO - is this legit?????????????????????????????????????????????????????????????????????????????????????????????????????????
        ...
        if node_label is True:
            ...
            for i in range(n):
                labels[i] = np.array([v for k,v in nx.degree(graph_list[i])])
                #print(nx.degree(graph_list[i]))
                
                #import pylab
                #nx.draw_networkx(graph_list[i], arrows=True)
                #pylab.show()
        ...
        while it < h:
            ...
            for i in range(n):
                for v in range(len(lists[i])):
                    ...
                    index_array = [int(elem) for elem in lists[i][v]]
                    long_label = np.concatenate((np.array([labels[i][v]]),
                                                 np.sort(labels[i][index_array])))
